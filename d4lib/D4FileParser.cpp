...
static void* task_init(d4_task_part_t* handle, void* extra_data) {
	...
	for(int i = 0; buckets[i].chrom != NULL; i ++) {
		...
		//printf("Task %s:%d-%d: Bucket: %d-%d (Actual %d %d)\n", name, l, r, il, ir, buckets[i].left, buckets[i].right);
...
static int task_proc(d4_task_part_t* handle, void* task_context, void* extra_data) {
	...
	for(pos = l; pos < r; ) {
		...
		for(int i = 0; i < count; i ++) {
			...
			if(ctx->buckets[current].right <= value_pos) current += 1;
...
int main() {
	...
	stat_result_t* result = (stat_result_t*)Java_org_broad_igv_d4_D4FileParser_d4_1run_1stat(&env, NULL, handle, 0, "1", 0, 250000000, 32);
	for(int i = 0; i < 32; i ++)
		printf("%lf %d\n", result->data[i].sum, result->data[i].count);
